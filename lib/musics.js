...
function maybeMapKoreanTitle(music) {
  const { title } = music;
  const mapped =
    musicsKo.find((ko) => ko.id === music.id)?.infos[0].title ??
...
export function mapArtistsOrAsIs(music) {
  ...
  if (info) {
    artistTranslation[music.lyricist] = info.lyricist;
    artistTranslation[music.composer] = info.composer;
    artistTranslation[music.arranger] = info.arranger;
    return {
