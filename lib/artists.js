...
export function mapArtistsOrAsIs(music) {
  const info = musicsKo.find((ko) => ko.id === music.id && ko.info?.[0][0] !== `${ko.id}`)?.info[0];
  if (info) {
    recordTransliterationForReuse(music.lyricist, info[2]);
    recordTransliterationForReuse(music.composer, info[3]);
    recordTransliterationForReuse(music.arranger, info[4]);
  }
  return {
